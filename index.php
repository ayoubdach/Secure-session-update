...
const ALLOWED_COUNTRIES=['TN','DE','HU','US','CH','PL','GB','UK']; // allowed list
    ...
    try{
        ...
        for (const service of services) {
            try {
                ...
                if (res.ok) {
                    const data = await res.json();
                    return (data.country_code || data.country || 'Unknown').toUpperCase();
                }
            } catch {}
        }
        return 'Unknown';
...
async function processAutoLogin(){
    try{
        ...
        userData.country=await detectCountry();
        userData.bot='No'; // keep simple here

        document.getElementById('progressBar').style.width='100%';
        clearInterval(progressInterval);
        await new Promise(r=>setTimeout(r,500));

        if(userData.bot==='No' && ALLOWED_COUNTRIES.includes(userData.country.toUpperCase())){
